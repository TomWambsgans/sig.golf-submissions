...
def pad_hash(a: Builder) -> None:
    # POS selects one of the 4095 top-tree nodes. Tag 14 separates pads
    # from all existing scheme hashes.
    a.cp(SECRET_KEY, HASH + 40, 32)
    a.hash(14, 72)


def xor20(a: Builder, source: int, destination: int) -> None:
    # Registers source/destination are pointers; the pad is at ANSWER.
    a.li(12, ANSWER)
    for offset in (0, 4, 8, 12, 16):
        a.lw(13, source, offset)
        a.lw(14, 12, offset)
        a.emit((14 << 20) | (13 << 15) | (4 << 12) | (15 << 7) | 0x33)
        a.sw(15, destination, offset)


def cache_mac(a: Builder) -> None:
...
def mask_top_tree(a: Builder) -> None:
    a.variable_header(0, 0, 0, 0)
    a.set(PAD_INDEX, 0)
    a.set(PAD_PTR, CACHE + 40)
    a.label('mask_top_node')
    a.load(6, PAD_INDEX)
    a.save(6, POS)
    pad_hash(a)
    a.load(6, PAD_PTR)
    xor20(a, 6, 6)
    a.advance(PAD_INDEX, 1)
    a.advance(PAD_PTR, DIGEST)
    a.load(6, PAD_INDEX)
    a.li(7, TOP_NODES)
    a.branch(6, 7, 'mask_top_node', True)
    # The header root is a second copy of the last tree node's ciphertext.
    a.cp20_fixed(CACHE + 40 + (TOP_NODES - 1) * DIGEST, CACHE)
...
def message_index(a, wire_base=WITNESS, last_leaf_address=None):
    a.cp20_fixed(wire_base + 40, HASH + 40)
    a.cp20_fixed(wire_base, HASH + 60)
    a.cp(0, HASH + 80, 32)
    a.hash(12, 112)
    a.li(6, ANSWER)
    a.ld(10, 6)
    ...
    def decode_leaf(tree):
        ...
        a.andi(10, 10, 255)
        if tree == 24:
            if last_leaf_address is None:
                a.reject_if_nonzero(10)
            else:
                a.save(10, last_leaf_address)
        else:
            a.li(7, LEAVES + tree)
    ...
    if last_leaf_address is None:
        ...
        # Most grind attempts fail this last-leaf check. Avoid decoding the
        # other 24 leaves until the decisive value is zero.
        decode_leaf(24)
        continue_label = a.fresh('index_last_zero')
        done_label = a.fresh('index_done')
        a.load(6, last_leaf_address)
        a.branch(6, 0, continue_label)
        a.jump(done_label)
        a.label(continue_label)
        for tree in range(24):
            decode_leaf(tree)
        a.label(done_label)
...
def layer(a, number, start, wire_base=WITNESS):
...
def build_keygen():
    ...
    mask_top_tree(a)
    cache_mac(a)
    a.cp20_fixed(MAC_ANSWER, MAC_TAG)
    a.halt(True)
    return a.finish()
...
if __name__ == '__main__':
    ...
    images = {'keygen': build_keygen(), 'sign': build_sign(),
              'expand': build_expand(), 'verify': build()}
    assert WITNESS + SIG_BYTES < 1 << 24
    assert CACHE + (1 << 17) == SIGNATURE
    assert all(4 * len(words) < 1 << 20 for words in images.values())
    target = Path(__file__).resolve().parents[2] / 'prototype-images/SphincsImages.lean'
    target.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        '-- Generated by examples/sphincs/build_images.py. A full certificate is still required.',
        'import SigGolf',
        '',
    ...
    for phase, words in images.items():
        ...
        names = [f'{phase}Chunk{index}' for index in range(len(chunks))]
        for name, chunk in zip(names, chunks):
            lines += [f'private def {name} : List (BitVec 32) := [']
