...
theorem pad64_input (fc : FCtx) (hfc : fc.ok) (lam : Nat) (hlam : lam < fc.h) (v : Val)
  ...
  split
  · rw [FCtx.node, pad64_nodeF _ _ _ _ _ _ _ (length_sib fc hfc lam hlam) hv]; simp_all
  · rw [FCtx.node, pad64_nodeF _ _ _ _ _ _ _ hv (length_sib fc hfc lam hlam)]; simp_all
...
theorem level_lt (fc : FCtx) (hfc : fc.ok) (lam : Nat) (hlam : lam + 1 < fc.h)
  ...
  have h12 : (r.toState s).getReg .x12 = BitVec.ofNat 64 (0x1E0 + 16 * bitOf fc.E (lam + 1)) := by
    ...
    congr 1; omega
  have hmem : r.st.mem = [(⟨none, 0x1C8⟩, stW 0x1C8 (.bin .srl (.reg .x23) (cwf (lam + 1)))),
       (⟨none, 0x1C0⟩, stW 0x1C0 (cwf (lam + 1))),
  ...
  have m1C8 : (r.toState s).getMem (BitVec.ofNat 64 0x1C8) =
    ...
    simp only [stW, Rv.E.eval, BinOp.eval, cwf]
    rw [← Rv.E.eval, srl_eval fc.E (lam + 1) hE10 (by omega) s h23]
  ...
  · have := hst; simp only [hr, foldExp, if_neg (show ¬ (lam + 1 = fc.h) by omega)] at this
    exact this
  ...
  · rw [hashInput_ofNat _ 0x1C0 0 h10 h11 (by decide) (by decide),
    ...
    rw [hlo, hhi, m1C0, m1C8, mfr 0x1D0 (by omega) (by omega) (by omega) (by omega) (by omega),
      ...
      hP 0x1D8 (by decide)]
    rcases Nat.lt_two_iff.mp hb2 with h0 | h1
    · rw [h0] at mv0 mv1 msib0 msib1
      simp only [Nat.mul_zero, Nat.add_zero, Nat.sub_zero] at mv0 mv1 msib0 msib1
      rw [mv0, mv1, msib0, msib1]; simp [h0]
    · rw [h1] at mv0 mv1 msib0 msib1
      simp only [Nat.mul_one] at mv0 mv1 msib0 msib1
      rw [show (0x1E0 : Nat) = 0x1F0 - 16 by rfl] at msib0
      rw [show (0x1E8 : Nat) = 0x1F0 - 16 + 8 by rfl, show (0x1F0 : Nat) = 0x1E0 + 16 by rfl, show (0x1F8 : Nat) = 0x1E8 + 16 by rfl]
      rw [mv0, mv1, msib0, msib1]; simp [h1]
  · intro a
    have hb' := bitOf_lt fc.E (lam + 1)
    ...
    refine ⟨Glob_writeHash (hglob _ _ hG) a _ h12 (by
        rcases Nat.lt_two_iff.mp hb' with h | h <;> rw [h] <;> decide),
    ...
    · rw [writeHash_pc, PRes.toState_pc]
      simp only [hr, foldExp, if_neg (show ¬ (lam + 1 = fc.h) by omega)]
      rw [pcOf_add4]
      exact congrArg pcOf (by unfold foldPc; split <;> split <;> omega)
