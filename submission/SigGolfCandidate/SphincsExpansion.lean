...
theorem loop_block (s : MachineState) (pc : s.pc = 0x1014)
...
theorem loop_next_pc (s : MachineState) (pc : s.pc = 0x1014) :
    (loopNext s).pc = if s.getReg .x10 = 1 then 0x102c else 0x1014 := by
...
def Invariant (n : Nat) (s : MachineState) : Prop :=
  n ≤ 1415 ∧ s.pc = (if n = 0 then 0x102c else 0x1014) ∧
  s.getReg .x6 = BitVec.ofNat 64 (0x20060 + 8 * (1415 - n)) ∧
  s.getReg .x7 = BitVec.ofNat 64 (0x22ca0 + 8 * (1415 - n)) ∧
...
theorem loop_invariant (n : Nat) (s : MachineState) (inv : Invariant (n + 1) s) :
  ...
  have hp : s.pc = 0x1014 := by simpa using pc
  ...
  · rw [(loop_next_regs s).1, src]
    change BitVec.ofNat 64 (0x20060 + 8 * (1415 - (n + 1))) + BitVec.ofNat 64 8 = _
    rw [← BitVec.ofNat_add]
    congr 1
    omega
  · rw [(loop_next_regs s).2.1, dst]
    change BitVec.ofNat 64 (0x22ca0 + 8 * (1415 - (n + 1))) + BitVec.ofNat 64 8 = _
    rw [← BitVec.ofNat_add]
    congr 1
    omega
...
def prefixState (s : MachineState) : MachineState :=
  ...
  let s := execInstrBr s (.LUI .x7 0x23)
  let s := execInstrBr s (.ADDI .x7 .x7 (-864))
  execInstrBr s (.ADDI .x10 .x0 1415)

theorem prefix_block (s : MachineState) (pc : s.pc = 0x1000) :
    OrdinarySteps expand s 5 (prefixState s) := by
  let s1 := execInstrBr s (.LUI .x6 0x20)
  let s2 := execInstrBr s1 (.ADDI .x6 .x6 0x60)
  let s3 := execInstrBr s2 (.LUI .x7 0x23)
  let s4 := execInstrBr s3 (.ADDI .x7 .x7 (-864))
  let s5 := execInstrBr s4 (.ADDI .x10 .x0 1415)
  ...
  · simp only [fetch, s1, execInstrBr, MachineState.setPC, pc, expand]; decide
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.LUI .x7 0x23)) 2
  · simp only [fetch, s1, s2, execInstrBr, MachineState.setPC, pc, expand]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.ADDI .x7 .x7 (-864))) 1
  · simp only [fetch, s1, s2, s3, execInstrBr, MachineState.setPC, pc, expand]; decide
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.ADDI .x10 .x0 1415)) 0
  · simp only [fetch, s1, s2, s3, s4, execInstrBr, MachineState.setPC, pc, expand]; decide
  · rfl
  exact OrdinarySteps.refl _

theorem prefix_invariant (s : MachineState) (pc : s.pc = 0x1000) :
    Invariant 1415 (prefixState s) := by
...
def tailState (s : MachineState) : MachineState :=
  execInstrBr (execInstrBr s (.LWU .x11 .x6 0)) (.SW .x7 .x11 0)

theorem tail_block (s : MachineState) (pc : s.pc = 0x102c)
    (src : accessValid (s.getReg .x6) 4 = true)
    (dst : accessValid (s.getReg .x7) 4 = true) :
    OrdinarySteps expand s 2 (tailState s) := by
  let s1 := execInstrBr s (.LWU .x11 .x6 0)
  let s2 := execInstrBr s1 (.SW .x7 .x11 0)
  apply OrdinarySteps.step s s1 _ (.base (.LWU .x11 .x6 0)) 1
  · simp only [fetch, pc, expand]; decide
  · simp [s1, ordinaryStep, memoryArgumentsValid, signExtend12, src]
  apply OrdinarySteps.step s1 s2 _ (.base (.SW .x7 .x11 0)) 0
  · simp only [fetch, s1, execInstrBr, MachineState.setPC, pc, expand]; decide
  · have hd : accessValid (s1.getReg .x7) 4 = true := by
      simpa [s1, execInstrBr, signExtend12, MachineState.getReg, MachineState.setReg,
        MachineState.setPC] using dst
    simp [s2, ordinaryStep, memoryArgumentsValid, signExtend12, hd]
  exact OrdinarySteps.refl _

theorem tail_pc (s : MachineState) (pc : s.pc = 0x102c) :
    (tailState s).pc = 0x1034 := by
  simp [tailState, execInstrBr, pc]

def finishState (s : MachineState) : MachineState :=
  execInstrBr (execInstrBr s (.ADDI .x5 .x0 0)) (.ADDI .x10 .x0 1)

theorem finish (hash : Hash) (s : MachineState) (pc : s.pc = 0x1034) :
...
theorem tail_accesses (s : MachineState) (inv : Invariant 0 s) :
    accessValid (s.getReg .x6) 4 = true ∧ accessValid (s.getReg .x7) 4 = true := by
  obtain ⟨_, _, src, dst, _⟩ := inv
  simp [accessValid, rangeValid, src, dst, BitVec.toNat_ofNat,
    MEMORY_BYTES]

theorem tail_executes (hash : Hash) (s : MachineState) (inv : Invariant 0 s) :
    Executes hash expand s 5 ⟨.success, finishState (tailState s), 5, 0, 0⟩ := by
  have block := tail_block s (by simpa using inv.2.1)
    (tail_accesses s inv).1 (tail_accesses s inv).2
  have tail := finish hash (tailState s) (tail_pc s (by simpa using inv.2.1))
  simpa [Execution.charge] using block.then_executes tail


theorem loop_executes (hash : Hash) (n : Nat) (s : MachineState) (inv : Invariant n s) :
    ∃ final, Executes hash expand s (6 * n + 5)
      ⟨.success, final, 6 * n + 5, 0, 0⟩ := by
  induction n generalizing s with
  | zero => exact ⟨finishState (tailState s), tail_executes hash s inv⟩
  | succ n ih =>
...
/-- The exact expansion image succeeds in 8,500 cycles and makes no oracle calls. -/
theorem executes (hash : Hash) (s : MachineState) (pc : s.pc = 0x1000) :
    ∃ final, Executes hash expand s 8500 ⟨.success, final, 8500, 0, 0⟩ := by
  obtain ⟨final, tail⟩ := loop_executes hash 1415 (prefixState s)
    (prefix_invariant s pc)
  refine ⟨final, ?_⟩
