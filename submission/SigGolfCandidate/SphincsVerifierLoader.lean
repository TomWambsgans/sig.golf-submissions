...
theorem loaded_message (publicKey : SigGolf.PublicKey) (message : Message)
    (witness : Bytes SphincsWire.signatureBytes) (state : MachineState)
    (loaded : initialState submission .verify (message, publicKey, witness) =
      some state)
    (i : Nat) (hi : i < 32) :
    state.getByte (BitVec.ofNat 64 i) =
  ...
  rw [show Riscv.witnessBase
    ⟨SphincsWire.signatureBytes, SphincsWire.signatureBytes⟩ = 0x22ca0 by decide]
  have skipWitness (before : MachineState) :=
    Memory.write_preserves_byte before 0x22ca0 (bytes witness) 0 i
      (by decide)
      (by rw [Memory.bytes_length (n := SphincsWire.signatureBytes)]; decide)
      (by omega) (by left; omega)
  simp only [Nat.zero_add] at skipWitness
  rw [skipWitness]
  have skipKey (before : MachineState) :=
    Memory.write_preserves_byte before 0x40 (bytes publicKey) 0 i
      (by decide)
  ...
  simpa only [Nat.zero_add] using
    Memory.write_value_byte _ 0 32 message i
      (by decide) (by decide) hi
...
theorem loaded_message_word (publicKey : SigGolf.PublicKey)
    (message : Message) (witness : Bytes SphincsWire.signatureBytes)
    (state : MachineState)
    (loaded : initialState submission .verify (message, publicKey, witness) =
      some state)
    (index : Fin 4) :
    state.getMem (BitVec.ofNat 64 (8 * index.val)) =
      message.extractLsb' (64 * index.val) 64 := by
  apply eq_of_forall_extractByte
  intro byte hbyte
  have h := loaded_message publicKey message witness state loaded
    (8 * index.val + byte) (by have := index.isLt; omega)
  have word := getByte_word state 0 (8 * index.val + byte)
    (by decide) (by omega)
  simp only [Nat.zero_add] at word
  rw [word] at h
  have quotient : (8 * index.val + byte) / 8 = index.val := by omega
  have remainder : (8 * index.val + byte) % 8 = byte := by omega
  simp only [quotient, remainder] at h
  ...
  symm
  simpa only [quotient, remainder] using
    extractByte_slice message (8 * index.val + byte)

/-- info: 'SigGolfCandidate.SphincsVerifierLoader.loaded_message_word' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms loaded_message_word
