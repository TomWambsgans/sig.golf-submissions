import SigGolfCandidate.SphincsVerifierWotsOuterFrame

namespace SigGolfCandidate.SphincsVerifierWotsOuterLoop
...
theorem stepLoop_to_chain_end_frame (hash : Hash) (start : Fin 8)
    (state : MachineState)
    (pc : state.pc = 0x2774)
  ...
  have next (i : Nat) (current : MachineState)
    ...
    · refine ⟨stepRound_pc hash current digit inv.1 currentCell small,
      ...
      · intro address control
        exact (stepRound_controlFrame hash current address control).trans
...
/-- info: 'SigGolfCandidate.SphincsVerifierWotsOuterLoop.stepLoop_to_chain_end_frame' depends on axioms: [propext,
 Classical.choice,
 Quot.sound] -/
#guard_msgs in
#print axioms stepLoop_to_chain_end_frame

end SigGolfCandidate.SphincsVerifierWotsOuterLoop
