import SigGolfCandidate.SphincsVerifierFtsTreeTag

/-! Identify the FORS parent HASH query for any level and bounded path pointer. -/

namespace SigGolfCandidate.SphincsVerifierFtsTreeQuery
...
open SigGolfCandidate.SphincsVerifierFtsTreeTag
...
def parentInputTree (pk : SphincsSecurity.PublicKey) (tree : FtsTree) (index : Index)
    (nodeIdx : FtsLeaf) (level : Nat) (left right : Digest) : HashInput :=
  tweakableHashInput pk.parameter (.ftsNode index tree level nodeIdx.val)
    (SphincsSecurity.Concrete.nodePayload left right)

theorem parentInputTree_eq (pk : SphincsSecurity.PublicKey) (tree : FtsTree) (index : Index)
    (nodeIdx : FtsLeaf) (level : Nat) (left right : Digest) :
    parentInputTree pk tree index nodeIdx level left right =
      fieldBytes (tweakFields 10 tree.val index.val level nodeIdx.val) ++
        bytesLE 20 pk.parameter ++ bytesLE 20 left ++ bytesLE 20 right := by
  rfl

theorem parentInputTree_length (pk : SphincsSecurity.PublicKey) (tree : FtsTree) (index : Index)
    (nodeIdx : FtsLeaf) (level : Nat) (left right : Digest) :
    (parentInputTree pk tree index nodeIdx level left right).length = 80 := by
  simp [parentInputTree, tweakableHashInput, tweakBytes,
    hashDomainFields, fieldBytes, SphincsSecurity.Concrete.nodePayload, bytesLE]

theorem parentInputTree_header (pk : SphincsSecurity.PublicKey) (tree : FtsTree) (index : Index)
    (nodeIdx : FtsLeaf) (level : Nat) (left right : Digest)
    (i : Nat) (hi : i < 20) :
    ((parentInputTree pk tree index nodeIdx level left right).map UInt8.toBitVec)[i]'(by
      rw [List.length_map, parentInputTree_length]; omega) =
      ((fieldBytes (tweakFields 10 tree.val index.val level nodeIdx.val)).map
        UInt8.toBitVec)[i]'(by simp [fieldBytes, bytesLE]; omega) := by
  simp only [parentInputTree_eq, List.map_append]
  rw [List.getElem_append_left (by simp [fieldBytes, bytesLE]; omega),
    List.getElem_append_left (by simp [fieldBytes, bytesLE]; omega),
    List.getElem_append_left (by simp [fieldBytes, bytesLE]; omega)]

theorem parentInputTree_parameter (pk : SphincsSecurity.PublicKey)
    (tree : FtsTree) (index : Index) (nodeIdx : FtsLeaf) (level : Nat)
    (left right : Digest) (i : Nat) (hi : i < 20) :
    ((parentInputTree pk tree index nodeIdx level left right).map UInt8.toBitVec)[20 + i]'(by
      rw [List.length_map, parentInputTree_length]; omega) =
...
theorem parentInputTree_left (pk : SphincsSecurity.PublicKey)
    (tree : FtsTree) (index : Index) (nodeIdx : FtsLeaf) (level : Nat)
    (left right : Digest) (i : Nat) (hi : i < 20) :
    ((parentInputTree pk tree index nodeIdx level left right).map UInt8.toBitVec)[40 + i]'(by
      rw [List.length_map, parentInputTree_length]; omega) =
...
theorem parentInputTree_right (pk : SphincsSecurity.PublicKey)
    (tree : FtsTree) (index : Index) (nodeIdx : FtsLeaf) (level : Nat)
    (left right : Digest) (i : Nat) (hi : i < 20) :
    ((parentInputTree pk tree index nodeIdx level left right).map UInt8.toBitVec)[60 + i]'(by
      rw [List.length_map, parentInputTree_length]; omega) =
...
theorem readyParent_hashInput_tree (state : MachineState)
    ...
    hashInput (parentHashReadyState state) =
      toQuery (parentInputTree pk tree index nodeIdx level left right) := by
  ...
  apply Serialization.hashInput_of_list ready 0x40000
    ((parentInputTree pk tree index nodeIdx level left right).map UInt8.toBitVec)
  · exact regs.1
  · change ((hashRegistersState (parentReadyState state)).getReg .x11).toNat = _
    rw [regs.2.1, List.length_map, parentInputTree_length]
    rfl
  · intro i hi
    have hi80 : i < 80 := by
      simpa [parentInputTree_length] using hi
    by_cases header : i < 20
    · let byte : Fin 20 := ⟨i, header⟩
      have actual := parentHeader_bytes_tree state tree index nodeIdx level
        treeValue positionValue treeIndex nodeIndex byte
      have expected := parentInputTree_header pk tree index nodeIdx level
        left right i header
    ...
    · by_cases parameter : i < 40
      · have smaller : i - 20 < 20 := by omega
        ...
        have expected := parentInputTree_parameter pk tree index nodeIdx level
      ...
      · by_cases leftPart : i < 60
        · have smaller : i - 40 < 20 := by omega
          ...
          have expected := parentInputTree_left pk tree index nodeIdx level
        ...
        · have smaller : i - 60 < 20 := by omega
          ...
          have expected := parentInputTree_right pk tree index nodeIdx level
...
/-- info: 'SigGolfCandidate.SphincsVerifierFtsTreeQuery.readyParent_hashInput_tree' depends on axioms: [propext,
 Classical.choice,
 Quot.sound] -/
#guard_msgs in
#print axioms readyParent_hashInput_tree

end SigGolfCandidate.SphincsVerifierFtsTreeQuery
