...
/-- A chain-step block holds the header, the tree index, and the 16-byte value twice. -/
def inputWord (head : Word) (tree : Nat) (value : Reference.Digest) (i : Fin 8) : Word :=
  if i.val = 0 then head
  else if i.val < 4 then (BitVec.ofNat 192 tree).extractLsb' (64 * (i.val-1)) 64
  else value.extractLsb' (64 * ((i.val-4) % 2)) 64

def payload (head : Word) (tree : Nat) (value : Reference.Digest) : List Byte :=
  bytes (n := 8) head ++ bytes (n := 24) (BitVec.ofNat 192 tree) ++ Reference.chainPayload value

@[simp] theorem payload_length (head : Word) (tree : Nat) (value : Reference.Digest) :
    (payload head tree value).length = 64 := by simp [payload,Reference.chainPayload,bytes]

theorem payload_byte (head : Word) (tree : Nat) (value : Reference.Digest) (i : Fin 64) :
    extractByte (inputWord head tree value ⟨i.val/8,by have := i.isLt; omega⟩) (i.val%8) =
      (payload head tree value)[i.val]'(by simp) := by
  fin_cases i <;> simp [inputWord,payload,Reference.chainPayload,bytes,List.getElem_append]
  all_goals
    ext j hj
...
theorem query_eq (s : MachineState) (head : Word) (tree : Nat) (value : Reference.Digest)
    ...
    (words : ∀ i : Fin 8, s.getMem (Signing.wordAddress 0x80000 i.val) = inputWord head tree value i) :
...
theorem answer_words (hash : Hash) (s : MachineState)
    ...
    ∀ i : Fin 2, (writeHash s (hash (hashInput s))).getMem (Signing.wordAddress 0x80300 i.val) =
      (Reference.truncate (Reference.query hash tag level tree leaf chain step
        (Reference.chainPayload value))).extractLsb' (64*i.val) 64 := by
  intro i
  rw [Signing.hash_answer_word s (hash (hashInput s)) destination ⟨i.val,by have := i.isLt; omega⟩]
  have eq : hash (hashInput s) = Reference.query hash tag level tree leaf chain step
      (Reference.chainPayload value) := by
    rw [query_eq s _ tree value source bits words]
    rfl
  rw [eq]
  let result := Reference.query hash tag level tree leaf chain step (Reference.chainPayload value)
...
/-- Eight concrete memory words determine the complete 64-byte chain-step block. -/
theorem words_of_layout (s : MachineState) (head : Word) (tree : Nat) (value : Reference.Digest)
    ...
    (hkept : ∀ i : Fin 2, s.getMem (Signing.wordAddress 0x80030 i.val) =
      value.extractLsb' (64*i.val) 64) :
    ∀ i : Fin 8, s.getMem (Signing.wordAddress 0x80000 i.val) = inputWord head tree value i := by
  have h0 := hindex 0
  have h1 := hindex 1
  have h2 := hindex 2
  have v0 := hvalue 0
  ...
  have k0 := hkept 0
  have k1 := hkept 1
  norm_num [Signing.wordAddress] at h0 h1 h2 v0 v1 k0 k1
  intro i
...
theorem secret_query_eq (s : MachineState) (head : Word) (tree : Nat) (secretKey : SecretKey)
  ...
  · rw [secretPayload_length]
  · rw [secretPayload_length]; decide
  · intro i hi
    have bound : i < 64 := by simpa using hi
    rw [Signing.getByte_word s 0x80000 i (by decide) (by omega),words ⟨i/8,by omega⟩]
