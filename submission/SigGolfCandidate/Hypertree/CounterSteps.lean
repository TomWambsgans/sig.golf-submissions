...
def RecurrentCode (image : Image) (p : Word) : Prop :=
  CounterArgs.Code image p ∧ CounterCore.Code image (p+100)
...
theorem initial (image : Image) (hash : Hash)
  ...
  obtain ⟨final,run,finalPC,finalData,finalReady,finalBase,finalConstant,finalArgs,finalCounter,ra,sp,frame⟩ :=
  ...
  refine ⟨final,(CheckReuse.block image 0x14f4 checkCode s pc base).trace.trans run,finalPC,
    finalData,finalReady,finalBase,finalConstant,finalArgs,finalCounter,ra.trans (CheckReuse.short_stack s).1,
...
theorem recurrent (image : Image) (hash : Hash)
    ...
    (prepareCode : CounterArgs.Code image 0x1588) (coreCode : CounterCore.Code image 0x15ec)
    (s : MachineState) (level tree step : Nat) (side : Bool) (chain : Reference.Chain) (value : Reference.Digest)
    ...
    ∃ final, Trace hash image s 13 20 1 1 final ∧ final.pc = 0x1580 ∧
      Buffered final level tree side chain (step+1) (Reference.chainHash hash level tree side chain step value) ∧
      CachedPrepare.Ready final ∧ final.getReg .x28 = 0x80438 ∧ final.getReg .x13 = 4294967296 ∧
      (final.getReg .x11 = 384 ∧ final.getReg .x12 = 0x80020 ∧ final.getReg .x5 = 1) ∧
      final.getReg .x6 = final.getMem 0x80438 ∧
  ...
  have ne : s.getMem 0x80438 ≠ 7 := by
    ...
    change step % 2^64 = 7 at h
    omega
  have checkedPC : (CounterCheck.shortCheck s).pc = 0x1588 := by
    rw [CounterCheck.pc s counter,pc,if_neg ne]; rfl
  ...
  obtain ⟨final,run,finalPC,finalData,finalReady,finalBase,finalConstant,finalArgs,finalCounter,ra,sp,frame⟩ :=
    CounterData.recurrent image hash 0x1588 prepareCode coreCode (CounterCheck.shortCheck s)
  ...
  refine ⟨final,(CounterCheck.block image 0x1580 checkCode s pc).trace.trans run,finalPC,
    finalData,finalReady,finalBase,finalConstant,finalArgs,finalCounter,ra.trans (CounterCheck.short_stack s).1,
