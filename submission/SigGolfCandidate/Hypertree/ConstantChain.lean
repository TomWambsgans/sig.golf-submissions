...
namespace SigGolfCandidate.Hypertree.ConstantChain
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 Keygen
set_option maxRecDepth 50000

def ChainCode (image : Image) (p : Word) : Prop :=
  ConstantPrepare.Code image p ∧
  instructionAt image (p+96) = some (.base .ECALL) ∧ ConstantFinish.Code image (p+100)

theorem chain_compute (image : Image) (hash : Hash) (p : Word) (code : ChainCode image p)
    (s : MachineState) (pc : s.pc = p) (base : s.getReg .x28 = 0x80438) (constant : s.getReg .x13 = 4294967296) (ready : CachedPrepare.Ready s) (level tree step : Nat)
    ...
    ∃ final, Trace hash image s 23 30 1 1 (ChainLoopControl.increment final (-184)) ∧ final.pc = p+144 ∧
  ...
  let prepared := ConstantPrepare.state s
  have headTrace : OrdinarySteps image s 13 prepared :=
    ConstantPrepare.block image p code.1 s pc base
  have hpc : prepared.pc = p+96 := by
    simp only [prepared,ConstantPrepare.pc,pc]
  obtain ⟨preparedBase,source,bits,destination,service⟩ := ConstantPrepare.regs s base constant
  ...
  have words (i : Fin 6) :=
    (ConstantPrepare.header_memory_equiv s base constant ready (Signing.wordAddress 0x80000 i.val)).trans (oldwords i)
  ...
  have hashPC : hashed.pc = p+100 := by simp only [hashed,hash_pc,hpc]; simp [BitVec.add_assoc]
  ...
  have fpc : final.pc = p+100+44 := by simpa only [final,hashPC] using outpc
  ...
  have hashBase : hashed.getReg .x28 = 0x80018 := by
    ...
    exact preparedBase
  have post := ConstantFinish.block image (p+100) code.2.2 hashed hashPC hashBase
  ...
  · exact fra.trans ((hash_registers _ _ _).trans (ConstantPrepare.stack s).1)
  · exact fsp.trans ((hash_registers _ _ _).trans (ConstantPrepare.stack s).2)
  · intro a inputOutside answerOutside valueOutside
    ...
    exact ConstantPrepare.frame s base constant a (inputOutside 0) (inputOutside 4) (inputOutside 5)
  ...
  · change CachedPrepare.Ready (ChainLoopControl.increment (Copy6.optimized hashed 0x300 0x510) (-184))
    ...
    exact ConstantInvariant.hash_current prepared _ destination (ConstantInvariant.prepare_current s base constant ready)
  ...
  · change (ChainLoopControl.increment (Copy6.optimized hashed 0x300 0x510) (-184)).getReg .x13 = _
    ...
    exact (ConstantRegister.ConstantPrepare_preserves s).trans constant

/-- info: 'SigGolfCandidate.Hypertree.ConstantChain.chain_compute' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms chain_compute
end SigGolfCandidate.Hypertree.ConstantChain
