...
namespace SigGolfCandidate.Hypertree.AddressReuseChain
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 Keygen FastCopy16
set_option maxRecDepth 50000
set_option maxHeartbeats 2000000

theorem copy_input (image : Image) (p : Word) (code : AddressReuseProof.inputCode image p)
    (s : MachineState) (pc : s.pc = p) :
    ∃ final, OrdinarySteps image s 9 final ∧ final.pc = p+44 ∧
  ...
  refine ⟨optimized s 0x510 0x20, by simpa only [AddressReuseProof.input_equiv] using AddressReuseProof.input_block image p code s pc, by simpa [pc] using hp, values, ra, sp, ?_⟩
  intro a h
  apply frame a
  · simpa [Signing.wordAddress] using h 0
  · simpa [Signing.wordAddress] using h 1

theorem copy_output (image : Image) (p : Word) (code : AddressReuseProof.outputCode image p)
    (s : MachineState) (pc : s.pc = p) :
    ∃ final, OrdinarySteps image s 9 final ∧ final.pc = p+44 ∧
  ...
  refine ⟨optimized s 0x300 0x510, by simpa only [AddressReuseProof.output_equiv] using AddressReuseProof.output_block image p code s pc, by simpa [pc] using hp, values, ra, sp, ?_⟩
  intro a h
  apply frame a
  · simpa [Signing.wordAddress] using h 0
  · simpa [Signing.wordAddress] using h 1
...
def ChainCode (image : Image) (p : Word) : Prop :=
  AddressReuseProof.inputCode image p ∧ AddressReuseProof.headerCode image (p+44) ∧
  instructionAt image (p+236) = some (.base .ECALL) ∧ AddressReuseProof.outputCode image (p+240)
...
theorem chain_compute (image : Image) (hash : Hash) (p : Word) (code : ChainCode image p)
    ...
    ∃ final, Trace hash image s 46 53 1 1 final ∧ final.pc = p+284 ∧
  ...
  have headTrace : OrdinarySteps image copied 27 (KeygenChainHeader.state copied) := by
    simpa only [AddressReuseProof.header_equiv] using AddressReuseProof.header_block image (p+44) code.2.1 copied cpc
...
/-- info: 'SigGolfCandidate.Hypertree.AddressReuseChain.chain_compute' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms chain_compute
end SigGolfCandidate.Hypertree.AddressReuseChain
