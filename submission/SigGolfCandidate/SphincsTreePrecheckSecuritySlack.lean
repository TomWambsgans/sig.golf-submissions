...
/-- Room for both setup-time and subsequent parent-collision targets. -/
theorem inflated_security_with_setup_aware_tree_precheck (q r : Nat)
    (hr : 1 ≤ r) (hcap : r ≤ 2 ^ 128) (hinflate : r ≤ 48 * q)
    (adversary : Adversary) (hbound : HasHashQueryBound scheme adversary r) :
    forgeAdvantage scheme adversary + (q : ENNReal) / 2 ^ 128 +
      (q : ENNReal) * 4094 / 2 ^ 160 ≤ (q : ENNReal) / 2 ^ 127 := by
  calc
    _ ≤ (r : ENNReal) / 2 ^ 140 + (q : ENNReal) / 2 ^ 128 +
          (q : ENNReal) * 4094 / 2 ^ 160 := by
      exact add_le_add
        (add_le_add
          ((RetainedResidual.forgeAdvantage_le_native_bound fixedReferenceDummy
            (fun _ _ _ => fixedReferenceDummyWord_valid) adversary r hbound hcap).trans
...
/-- The inherited raw-signer reduction leaves room for a 128-bit public-key
commitment collision, a 160-bit altered-cache MAC guess, and a 256-bit
hidden-key oracle contact. The separate game bridges must establish these
three event bounds. -/
theorem inflated_security_with_masked_cache_mac (q r : Nat)
    (hr : 1 ≤ r) (hcap : r ≤ 2 ^ 128) (hinflate : r ≤ 48 * q)
    (adversary : Adversary) (hbound : HasHashQueryBound scheme adversary r) :
    forgeAdvantage scheme adversary + (q : ENNReal) / 2 ^ 128 +
  ...
  calc
    _ ≤ (r : ENNReal) / 2 ^ 140 + (q : ENNReal) / 2 ^ 128 +
          (q : ENNReal) / 2 ^ 160 + (q : ENNReal) / 2 ^ 256 := by
      exact add_le_add
        (add_le_add
          (add_le_add
            ((RetainedResidual.forgeAdvantage_le_native_bound fixedReferenceDummy
              (fun _ _ _ => fixedReferenceDummyWord_valid) adversary r hbound hcap).trans
              (raw_native_bound_le_140 r hr hcap)) le_rfl) le_rfl) le_rfl
    _ ≤ (q : ENNReal) / 2 ^ 127 := by
      apply (ENNReal.toReal_le_toReal (by finiteness) (by finiteness)).mp
      simp (disch := finiteness) only [ENNReal.toReal_add, ENNReal.toReal_div,
...
/-- info: 'SphincsSecurity.Concrete.inflated_security_with_setup_aware_tree_precheck' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms SphincsSecurity.Concrete.inflated_security_with_setup_aware_tree_precheck

/-- info: 'SphincsSecurity.Concrete.inflated_security_with_masked_cache_mac' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms SphincsSecurity.Concrete.inflated_security_with_masked_cache_mac
