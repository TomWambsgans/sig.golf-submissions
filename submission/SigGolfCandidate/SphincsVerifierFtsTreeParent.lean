import SigGolfCandidate.SphincsVerifierFtsTreeQuery

/-! The full parent HASH input from a generic FORS path-step state. -/

namespace SigGolfCandidate.SphincsVerifierFtsTreeParent
...
theorem parent_hashInput_tree (start : MachineState)
    (pointer : Word)
    ...
    hashInput (firstParentReadyState start) =
      toQuery (parentInputTree pk tree index nodeIdx level
  ...
  have treeValueAtPosition : positioned.getMem 0x43000 =
  ...
  exact readyParent_hashInput_tree positioned pk tree index nodeIdx level
    (if (parityState start).getReg .x6 = 0 then current else sibling)
    (if (parityState start).getReg .x6 = 0 then sibling else current)
    treeValueAtPosition positionAtPosition treeAtPosition indexAtPosition
    (positioned_WitnessPrefix start pk witnessPrefix)
    (fun i hi => (children i hi).1)
    (fun i hi => (children i hi).2)

/-- info: 'SigGolfCandidate.SphincsVerifierFtsTreeParent.parent_hashInput_tree' depends on axioms: [propext,
 Classical.choice,
 Quot.sound] -/
#guard_msgs in
#print axioms parent_hashInput_tree

end SigGolfCandidate.SphincsVerifierFtsTreeParent
