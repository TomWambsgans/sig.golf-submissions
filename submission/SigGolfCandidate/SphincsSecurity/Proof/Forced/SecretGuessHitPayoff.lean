...
omit [Nonempty Value] in
theorem lazyRun_guess_payoff_budget_event_le_sum {Result : Type}
    (environment : Environment auxSpec Coordinate Value Memory)
    (computation : OracleComp (World auxSpec Coordinate Value) Result)
    (memory : Memory) (budget : Nat)
    (payoff : Result × State Coordinate Value Memory → ENNReal) :
    (∑' result, Pr[= result | lazyRun environment computation (initialState memory)] *
      (if result.2.guesses.Nonempty ∧ result.2.probes ≤ budget then
        payoff result else 0)) ≤
      ∑ slot ∈ Finset.range budget, ∑' result,
        Pr[= result | hitRun environment slot computation (initialState memory)] *
          payoff result := by
  rw [← Summable.tsum_finsetSum (fun _ _ => ENNReal.summable)]
  apply ENNReal.tsum_le_tsum
  intro result
  ...
  · rw [if_pos hg]
    have h := lazyRun_new_guesses_le_sum environment computation
      (initialState memory) result budget hg.2
      (show result.2.guesses ≠
        (initialState memory : State Coordinate Value Memory).guesses
        from hg.1.ne_empty)
    have hp := mul_le_mul' h (le_rfl : payoff result ≤ payoff result)
    simpa only [initialState, ← Finset.range_eq_Ico, Finset.sum_mul,
      SPMF.probOutput_eq_apply] using hp
  · simp only [if_neg hg, mul_zero]
    exact bot_le

theorem lazyRun_guess_payoff_budget_event_le_forced {Result : Type}
    (environment : Environment auxSpec Coordinate Value Memory)
    (computation : OracleComp (World auxSpec Coordinate Value) Result)
    (memory : Memory) (budget : Nat)
    (payoff : Result × State Coordinate Value Memory → ENNReal) :
    (∑' result, Pr[= result | lazyRun environment computation (initialState memory)] *
      ...
      ((Fintype.card Value - budget : Nat) : ENNReal)⁻¹ *
        ∑ slot ∈ Finset.range budget, ∑' result,
          Pr[= result | forcedRun environment slot computation (initialState memory)] *
            payoff result := by
  apply (lazyRun_guess_payoff_budget_event_le_sum environment computation memory
    budget payoff).trans
  rw [Finset.mul_sum]
  apply Finset.sum_le_sum
  intro slot hslot
  exact hitRun_payoff_le_forced environment budget slot
    (Finset.mem_range.mp hslot).le computation memory payoff

theorem lazyRun_event_budget_le_forced {Result : Type}
    (environment : Environment auxSpec Coordinate Value Memory)
    (computation : OracleComp (World auxSpec Coordinate Value) Result)
    (memory : Memory) (budget : Nat)
    ...
    (payoff : Result × State Coordinate Value Memory → ENNReal)
    (hevent : ∀ result,
      lazyRun environment computation (initialState memory) result ≠ 0 →
      event result → result.2.guesses.Nonempty ∧ 1 ≤ payoff result) :
    Pr[fun result => event result ∧ result.2.probes ≤ budget |
  ...
  apply (probEvent_le_tsum_probOutput_mul_cost_of_mem_support _ _
    (fun result =>
      if result.2.guesses.Nonempty ∧ result.2.probes ≤ budget then
        payoff result else 0) ?_).trans
    (lazyRun_guess_payoff_budget_event_le_forced environment computation
      memory budget payoff)
  intro result hr he
  have h := hevent result
    (by simpa only [mem_support_iff, SPMF.probOutput_eq_apply] using hr) he.1
  simpa only [if_pos (show result.2.guesses.Nonempty ∧ result.2.probes ≤ budget
    from ⟨h.1, he.2⟩)] using h.2

end SphincsSecurity.Concrete.SecretGuessObservation

/-- info: 'SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_guess_payoff_budget_event_le_sum' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_guess_payoff_budget_event_le_sum

/-- info: 'SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_guess_payoff_budget_event_le_forced' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_guess_payoff_budget_event_le_forced

/-- info: 'SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_event_budget_le_forced' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms SphincsSecurity.Concrete.SecretGuessObservation.lazyRun_event_budget_le_forced
