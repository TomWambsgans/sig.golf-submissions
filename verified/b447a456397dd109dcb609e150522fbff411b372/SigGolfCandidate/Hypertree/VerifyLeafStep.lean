...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (26*(7-(Reference.digit message chain).val)+constantOverhead (7-(Reference.digit message chain).val)+44)
      (33*(7-(Reference.digit message chain).val)+constantOverhead (7-(Reference.digit message chain).val)+44)
