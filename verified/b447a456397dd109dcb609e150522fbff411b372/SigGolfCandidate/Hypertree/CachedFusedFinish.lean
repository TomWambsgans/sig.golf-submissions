...
namespace SigGolfCandidate.Hypertree.CachedFusedFinish
...
def state (s : MachineState) : MachineState :=
  ...
  execInstrBr s (.JAL .x0 (-156))
theorem state_equiv (s : MachineState) :
    state s = ChainLoopControl.increment (Copy6.optimized s 0x300 0x510) (-192) := by
  cases s
  ...
  funext r
  cases r <;> rfl
end SigGolfCandidate.Hypertree.CachedFusedFinish
