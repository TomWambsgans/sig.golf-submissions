import SigGolfCandidate.Hypertree.ConstantChainLoop
...
theorem chain_from_digit (hash : Hash) (s : MachineState) (level tree : Nat)
    ...
    ∃ final, Trace hash verify s (26*(7-digit.val)+constantOverhead (7-digit.val)) (33*(7-digit.val)+constantOverhead (7-digit.val)) (7-digit.val) (7-digit.val) final ∧
      final.pc = 0x163c ∧
  ...
  constant_loop verify verify_chain_check verify_short_check verify_chain_code verify_cached_check verify_cached_code hash s level tree digit.val (7-digit.val) side chain value pc (by have := digit.isLt; omega) data
