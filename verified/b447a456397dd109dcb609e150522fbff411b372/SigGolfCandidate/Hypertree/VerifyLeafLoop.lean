...
theorem leaf_loop (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 721*remaining ∧ cycles + 33 * ChainCost.chainPrefix message start ≤ 33 * ChainCost.chainPrefix message 46 + 68*remaining ∧ calls ≤ 7*remaining ∧ final.pc = 0x1690 ∧
  ...
  | succ remaining ih =>
    ...
    have overheadBound := constantOverhead_le (7-(Reference.digit message chain).val)
    refine ⟨final, (26*(7-(Reference.digit message chain).val)+constantOverhead (7-(Reference.digit message chain).val)+44)+steps,
      (33*(7-(Reference.digit message chain).val)+constantOverhead (7-(Reference.digit message chain).val)+44)+cycles,
...
theorem recover_all_chains (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 33166 ∧ cycles ≤ 13292 ∧ calls ≤ 322 ∧ final.pc = 0x1690 ∧
      final.getMem 0x80430 = 46 ∧ LeafData final level tree side base message values ∧
