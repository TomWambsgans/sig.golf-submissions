...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (13*(7-(Reference.digit message chain).val)+counterOverhead (7-(Reference.digit message chain).val)+44)
      (20*(7-(Reference.digit message chain).val)+counterOverhead (7-(Reference.digit message chain).val)+44)
