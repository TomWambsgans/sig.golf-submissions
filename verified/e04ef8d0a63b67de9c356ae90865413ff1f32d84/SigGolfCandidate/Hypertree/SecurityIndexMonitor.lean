...
noncomputable def play : Strategy → Finset (BitVec 152) → Finset (BitVec 152) → Nat → ProbComp Bool
...
theorem draw_bound (targets : Finset (BitVec 152)) (next : BitVec 256 → ProbComp Bool)
    (work : BitVec 256 → ENNReal) (future initial limit : Nat)
    (upper : ∀ answer, Pr[fun hit => hit = true | next answer] ≤
      (future : ENNReal) / 2^152 + (limit : ENNReal) / 2^152 * work answer)
    (amortized : targets.card + future ≤ initial + limit) :
    Pr[fun hit => hit = true | (do
      let answer ← $ᵗ BitVec 256
      let later ← next answer
      pure (decide (answer.extractLsb' 0 152 ∈ targets) || later))] ≤
      (initial : ENNReal) / 2^152 + (limit : ENNReal) / 2^152 *
        (1 + expectedValue ($ᵗ BitVec 256) work) := by
  have first := SecurityGraphPassive.passive_or_le ($ᵗ BitVec 256)
    (fun answer => decide (answer.extractLsb' 0 152 ∈ targets)) next
  have density : Pr[fun answer : BitVec 256 => decide (answer.extractLsb' 0 152 ∈ targets) = true |
      $ᵗ BitVec 256] = (targets.card : ENNReal) / 2^152 := by
    simpa only [decide_eq_true_eq] using SecurityUniform.prob_extract_mem 104 152 targets
  rw [density] at first
  have rest := prob_bind_le ($ᵗ BitVec 256) next _ upper
  have integral : expectedValue ($ᵗ BitVec 256)
      (fun answer => (future : ENNReal)/2^152+(limit : ENNReal)/2^152*work answer) =
      (future : ENNReal)/2^152+(limit : ENNReal)/2^152*expectedValue ($ᵗ BitVec 256) work := by
    rw [expectedValue_add, expectedValue_const NeverFail.probFailure_eq_zero]
    simp only [mul_comm ((limit : ENNReal)/2^152), expectedValue_mul_const]
  rw [integral] at rest
  have arithmetic : ((targets.card : ENNReal)+(future : ENNReal))/2^152 ≤
      ((initial : ENNReal)+(limit : ENNReal))/2^152 := by
    apply ENNReal.div_le_div _ le_rfl
    exact_mod_cast amortized
  calc
    _ ≤ (targets.card : ENNReal)/2^152 + ((future : ENNReal)/2^152 +
      (limit : ENNReal)/2^152*expectedValue ($ᵗ BitVec 256) work) := first.trans (add_le_add le_rfl rest)
    _ = ((targets.card : ENNReal)+(future : ENNReal))/2^152 +
      (limit : ENNReal)/2^152*expectedValue ($ᵗ BitVec 256) work := by rw [ENNReal.add_div]; ac_rfl
    _ ≤ ((initial : ENNReal)+(limit : ENNReal))/2^152 +
      (limit : ENNReal)/2^152*expectedValue ($ᵗ BitVec 256) work := add_le_add arithmetic le_rfl
...
theorem prob_play_le (strategy : Strategy) (seen marked : Finset (BitVec 152)) (remaining limit : Nat)
    (allowance : marked.card + remaining ≤ limit) :
    Pr[fun hit => hit = true | play strategy seen marked remaining] ≤
      ((remaining * seen.card : Nat) : ENNReal)/2^152 + (limit : ENNReal)/2^152 * cost strategy := by
  ...
  | coin n next ih =>
    ...
    simpa only [mul_comm ((limit : ENNReal)/2^152), expectedValue_mul_const] using bound
  | draw mark next ih =>
    ...
    · simp only [play, if_pos marking, cost]
      ...
      · intro answer
        have nextAllowance : (insert (answer.extractLsb' 0 152) marked).card + (remaining-1) ≤ limit := by
          have bound := Finset.card_insert_le (answer.extractLsb' 0 152) marked
          omega
        exact (ih answer _ _ _ nextAllowance).trans (add_le_add
          (ENNReal.div_le_div (by exact_mod_cast (Nat.mul_le_mul_left (remaining-1)
              (Finset.card_insert_le (answer.extractLsb' 0 152) seen))) le_rfl) le_rfl)
...
/-- At most L marked signing draws spend at most L/2^152 per expected fresh H5 call. -/
theorem prob_empty_le (strategy : Strategy) (limit : Nat) :
    Pr[fun hit => hit = true | play strategy ∅ ∅ limit] ≤ (limit : ENNReal)/2^152 * cost strategy := by
...
theorem prob_lifetime_le (strategy : Strategy) :
  ...
  have rate : (LIFETIME : ENNReal)/2^152 = 1/(2:ENNReal)^128 := by
