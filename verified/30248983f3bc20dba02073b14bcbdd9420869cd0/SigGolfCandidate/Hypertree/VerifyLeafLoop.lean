...
theorem leaf_loop (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 721*remaining ∧ cycles + 66 * ChainCost.chainPrefix message start ≤ 66 * ChainCost.chainPrefix message 46 + 49*remaining ∧ calls ≤ 7*remaining ∧ final.pc = 0x1690 ∧
  ...
  | succ remaining ih =>
    ...
    refine ⟨final, (59*(7-(Reference.digit message chain).val)+49)+steps,
      (66*(7-(Reference.digit message chain).val)+49)+cycles,
...
theorem recover_all_chains (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 33166 ∧ cycles ≤ 22582 ∧ calls ≤ 322 ∧ final.pc = 0x1690 ∧
      final.getMem 0x80430 = 46 ∧ LeafData final level tree side base message values ∧
