...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (15*(7-(Reference.digit message chain).val)+inplaceOverhead (7-(Reference.digit message chain).val)+44)
      (22*(7-(Reference.digit message chain).val)+inplaceOverhead (7-(Reference.digit message chain).val)+44)
