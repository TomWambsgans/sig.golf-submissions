...
def WitnessStored (s : MachineState) (witness : Bytes signatureBytes) : Prop :=
  ∀ i, i < signatureBytes → s.getByte (BitVec.ofNat 64 (0x3bc30+i)) = witness.extractLsb' (8*i) 8
...
theorem WitnessStored.transfer (s final : MachineState) (witness : Bytes signatureBytes)
  ...
  rw [getByte_word final 0x3bc30 i (by decide) (by change i < 113616 at hi; omega),
    frame _ (by change (0x3bc30+8*(i/8)) % 2^64 < 0x80000; change i < 113616 at hi; omega),
    ← getByte_word s 0x3bc30 i (by decide) (by change i < 113616 at hi; omega)]
  exact stored i hi
...
theorem WitnessStored.word (s : MachineState) (witness : Bytes signatureBytes)
    ...
    s.getMem (BitVec.ofNat 64 (0x3bc30+offset)) = witness.extractLsb' (8*offset) 64 := by
  ...
  rw [getByte_word s 0x3bc30 (offset+j) (by decide) (by change offset+8 ≤ 113616 at bound; omega)] at h
...
theorem WitnessStored.slice_word (s : MachineState) (witness : Bytes signatureBytes)
    ...
    s.getMem (BitVec.ofNat 64 (0x3bc30+start+8*i.val)) =
...
theorem wire_layer_value (s : MachineState) (witness : Bytes signatureBytes)
    (stored : WitnessStored s witness) (level : Nat) (small : level < 152)
    (chain : Reference.Chain) (relevant : level ≠ 0 ∨ chain = 0) (i : Fin 2) :
    s.getMem (BitVec.ofNat 64 (0x3bc30+layerOffset level+16*chain.val+8*i.val)) =
  ...
  · have hc := chain.isLt
    have bound : 64+752*(level-1)+16*chain.val+16 ≤ signatureBytes := by change _ ≤ 113616; omega
...
theorem wire_layer_sibling (s : MachineState) (witness : Bytes signatureBytes)
    (stored : WitnessStored s witness) (level : Nat) (small : level < 152) (i : Fin 2) :
    s.getMem (BitVec.ofNat 64 (0x3bc30+layerOffset level+siblingOffset level+8*i.val)) =
  ...
  · have bound : 64+752*(level-1)+736+16 ≤ signatureBytes := by change _ ≤ 113616; omega
