...
namespace SigGolfCandidate.Hypertree.Copy6Chain
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 Keygen
set_option maxRecDepth 50000

def ChainCode (image : Image) (p : Word) : Prop :=
  Copy6.InputCode image p ∧ AddressReuseProof.headerCode image (p+44) ∧
...
theorem chain_compute (image : Image) (hash : Hash) (p : Word) (code : ChainCode image p)
    ...
    ∃ final, Trace hash image s 40 47 1 1 final ∧ final.pc = p+284 ∧
  ...
  obtain ⟨copied,pre,cpc,content,cra,csp,cframe⟩ :=
    Copy6.copy_input image p code.1 s pc
  ...
  obtain ⟨final,post,fpc,result,fra,fsp,fframe⟩ :=
    Copy6.copy_output image (p+240) code.2.2.2 hashed hashPC
...
/-- info: 'SigGolfCandidate.Hypertree.Copy6Chain.chain_compute' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms chain_compute
end SigGolfCandidate.Hypertree.Copy6Chain
