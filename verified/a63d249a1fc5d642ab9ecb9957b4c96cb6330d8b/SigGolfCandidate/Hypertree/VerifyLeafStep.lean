...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (28*(7-(Reference.digit message chain).val)+cachedOverhead (7-(Reference.digit message chain).val)+44)
      (35*(7-(Reference.digit message chain).val)+cachedOverhead (7-(Reference.digit message chain).val)+44)
