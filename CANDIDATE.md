...
- The exact expansion image and verifier prefix have compiled proofs. Parameterized RISC-V theorems compose all 24 FORS trees, preserve earlier roots, and account for their instructions and HASH calls. The abstract-root identification is conditional on the first-entry loader invariant, which is still being assembled.
- For a fixed oracle, canonical cached signing gives the same signature as the abstract signer. An exact six-layer theorem identifies the extra 855,635 hash calls when its top layer succeeds. A pathwise theorem preserves replies across adaptive signing requests and bounds the abstract signer’s extra hash calls. The accepted altered-cache split reduces the two signer checks to the honest case, a 160-bit top-node collision, or a 128-bit commitment collision.
