...
The scheme uses 11,324-byte signatures and witnesses, a 16-byte public-key commitment, and a proposed 171,399-cycle verification bound. The abstract Lean security proof and the exact expand-image proof build. The verifier proof currently covers its first commitment hash, comparison, witness-field copies, and proves that the next hash input contains the submitted message word for word. Full keygen/sign/verify correspondence, resource bounds, and the reduction for the committed public key and adversarial cache remain open.
