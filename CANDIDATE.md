...
- Prove security for the actual interface: the attacker sees the public top-tree cache, may replace it on each signing request, and may submit either a witness or a compact signature. The inherited theorem neither reveals this cache nor models the cached signer. The missing bridge must connect the canonical-cache coupling to the randomized game and handle adversary-modified caches. The current sign image can construct a hidden signature before rejecting a bad cache, which changes the inherited strong-SUF replay transcript. A public full-tree precheck is being prototyped in scratch so bad caches reject before signing; it is not yet part of this submission. Exact sign-bytecode checks and keygen/verifier refinements are also open.
